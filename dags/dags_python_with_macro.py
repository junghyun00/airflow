...
with DAG(
    ...
    @task(task_id = 'task_using_macros',
          templates_dict = { 'start_date': '{{data_interval_end.in_timezon("Asia/Seoul").subtract(month=1).replace(day=1) | ds}}'  # 전월 1일
                           , 'end_date'  : '{{data_interval_end.in_timezon("Asia/Seoul").subtract(month=1).end_of("month") | ds}}' # 전월 말일
          }
    )
    ...
    def get_datetime_calc(**kwargs):
        ...
        prev_month_day_first = data_interval_end.in_timezon("Asia/Seoul") + relativedelta(month = -1, day = 1)
        prev_month_day_last  = data_interval_end.in_timezon("Asia/Seoul").replace(day = 1) + relativedelta(days = -1)
