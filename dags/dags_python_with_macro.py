...
with DAG(
    ...
    @task(task_id = 'task_using_macros',
          templates_dict = { 'start_date': '{{data_interval_end.in_timezone("Asia/Seoul").subtract(month=1).replace(day=1) | ds}}'  # 전월 1일
                           , 'end_date'  : '{{data_interval_end.in_timezone("Asia/Seoul").subtract(month=1).end_of("month") | ds}}' # 전월 말일
          }
    )
